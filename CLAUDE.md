...
- **`[srx-gamelog]`** — JSON lines emitted when a completed SRX game's logs are auto-purged from the DB. One line per `TurnLog` row and one per `GameEvent` row, plus `session_log_dump_start` and `session_log_purge_complete` bookends. **`src/lib/session-log-export.ts`**.
- **`[ginrummy-gamelog]`** — same pattern for Gin Rummy; emitted by `games/ginrummy/src/definition.ts` inline (no shared export module needed). Written via `schedulePurge()` fire-and-forget after game ends (resigned, hand_complete single-hand, match_complete, timeout).
...
- **`tests/unit/engine-door-game.test.ts`** — covers game-agnostic `openFullTurn`, `closeFullTurn`, `tryRollRound` (forfeit slot handling, endgame tick on zero turns, day advancement)
...
3. **Per full turn:** `POST /api/game/tick` opens a turn (`openFullTurn` → `runAndPersistTick` with `decrementTurnsLeft: false`, `turnOpen: true`). Each successful mutating `POST /api/game/action` (not `end_turn`) triggers `doorGameAutoCloseFullTurnAfterAction` + `closeFullTurn` so one action closes the slot; explicit `end_turn` / **Skip** is for skipping without acting. **`closeFullTurn`** decrements that empire’s **`turnsLeft`** by 1 and increments **`turnsPlayed`** by 1 (one game turn per miniturn). **Round timer** forfeit: **`tryRollRound`** charges **`turnsLeft`** **and** increments **`turnsPlayed`** for each skipped daily slot (same accounting as closed full turns — `turnsPlayed + turnsLeft` always equals the original total). `tryRollRound` advances the calendar day when every empire has used all daily full turns, **or** after **`roundStartedAt + turnTimeoutSecs`** skips remaining slots (`round_timeout` event). `GET /api/game/status` calls `tryRollRound` so polling can complete a round.
...
- `src/lib/ai-job-queue.ts` — `AiTurnJob` CRUD: `enqueueAiTurnsForSession` (dedup: skips players with a pending/claimed job), `claimNextJob` (SKIP LOCKED; returns `retryCount`), `completeJob`, `failJob`, `recoverStaleJobs` (resets claimed jobs older than threshold back to pending for crashed-worker recovery; increments `retryCount` each reset; permanently fails jobs at `MAX_JOB_RETRIES = 3` to break infinite crash loops and returns their session IDs so the worker can re-enqueue fresh jobs).
- `src/lib/db-context.ts` — re-exports `getDb`, `withCommitLock`, `withAtomicWrites`, `SessionBusyError` from `@dge/engine/db-context`; registers the Prisma client with the engine. `withCommitLock(sessionId, fn)` uses `INSERT IGNORE SessionLock` + `SELECT … FOR UPDATE NOWAIT` (MySQL advisory lock). `withAtomicWrites(fn)` wraps the final DB writes (empire + army + planets + TurnLog) in a short transaction when not already inside a `withCommitLock` context — prevents orphaned state changes when a write fails mid-action in sequential mode. `GalaxyBusyError` is a deprecated alias for `SessionBusyError`.
...
- `scripts/ai-worker.ts` — standalone long-running process (separate Compose service). Polls `AiTurnJob` via `claimNextJob` (SKIP LOCKED), calls `runOneDoorGameAI`, then cascades with `enqueueAiTurnsForSession`. On failure: marks the job `failed` **and** immediately re-enqueues via `enqueueAiTurnsForSession` so the AI's daily slot is not silently lost. Supports `AI_WORKER_CONCURRENCY` parallel slots. Recovers stale (crashed-worker) jobs every 30 s; permanently fails jobs that have been stale-recovered `MAX_JOB_RETRIES` times and re-enqueues fresh jobs for those sessions (breaks OOM crash loops). Env: `DATABASE_URL`, `REDIS_URL`, `GEMINI_API_KEY`, `AI_WORKER_POLL_MS`, `AI_WORKER_CONCURRENCY`, `SRX_LOG_AI_TIMING`.
...
- `src/components/GinRummyGameScreen.tsx` — full in-game Gin Rummy UI; card table layout, face-down opponent hand, stock/discard piles, interactive hand with meld/deadwood highlight, knock/gin/layoff buttons, match scoring panel, hand-result overlay, AI polling. Exports `ginRankIdx`/`ginSuitIdx` helpers (unit-tested). Hand sort (RANK/SUIT buttons) and drag-to-reorder maintain a client-side `localOrder` reconciled with server `myCards`. `?` button opens `HelpModal` via `GET /api/game/help?game=ginrummy`.
...
- `AiTurnJob` — door-game AI turn job queue. One row per pending AI turn (fields: `sessionId`, `playerId`, `status` pending/claimed/done/failed, `workerId`, `result` JSON, `retryCount` Int, timestamps). Claimed via `SELECT … FOR UPDATE SKIP LOCKED` so multiple workers never double-claim. `retryCount` is incremented each time stale recovery resets a claimed job to pending; at `MAX_JOB_RETRIES = 3` the job is permanently failed instead of retried. Rows are deleted by `deleteGameSession` along with the session.
...
- **`src/components/GinRummyGameScreen.tsx`** — owns the full Gin Rummy in-game UI (card table, hand rendering, draw/discard/knock/gin/layoff actions, match scoring, hand sort/drag, help modal). Registered in `GAME_SCREEN_REGISTRY` as the `ginrummy` game's screen component. Exports `ginRankIdx` and `ginSuitIdx` for unit testing.
