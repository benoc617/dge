...
- **AI:** **`drainDoorGameAiTurns`** (via **`runDoorGameAITurns`**) runs until no AI still owes daily slots (guard cap). Eligible AIs are ordered by **fewest `fullTurnsUsedThisRound` first**, then **`turnOrder` ascending** — fair interleaving for simultaneous play. **`runDoorGameAITurns`** wraps the drain with **`doorAiInFlight`** so overlapping kicks from **`tryRollRound`** (post-`day_complete`), a human **`after()`**, and **`GET /api/game/status`** serialize per session. **`runDoorGameAITurns`** runs after a human action (background `after()`) and from **`GET /api/game/status`** when any AI still has daily full turns left (mid-round catch-up). **Parallel decide:** up to **`doorAiDecideBatchSize`** (defaults **4**; **`SystemSettings`** via **`/admin`**, else env **`DOOR_AI_DECIDE_BATCH_SIZE`**) eligible AIs have **`getAIMoveDecision`** (Gemini / MCTS / fallback) overlapped with **`Promise.all`** after each has **`openFullTurn`** applied **serially**; **`processAction`** / **`closeFullTurn`** still run **one empire at a time** in that order. Decisions are best-effort against the DB state before the wave’s applies; later applies may invalidate earlier decisions — invalid moves use the existing skip / **`end_turn`** path. **Concurrency caps (global):** **`geminiMaxConcurrent`** / **`doorAiMaxConcurrentMcts`** in **`SystemSettings`** (admin), else env **`GEMINI_MAX_CONCURRENT`** (default **4**) and **`DOOR_AI_MAX_CONCURRENT_MCTS`** (default **1**). Each AI move is capped by **`doorAiMoveTimeoutMs`** (admin), else env **`DOOR_AI_MOVE_TIMEOUT_MS`** (default **60s**). If `processAiMoveOrSkip` takes the **skip** path (invalid action → `end_turn`), **`closeFullTurn` must still run** — same as a direct `end_turn` — or `turnOpen` stays true, `fullTurnsUsedThisRound` never increments, and the round can stall (`applyDoorGameAIMove` pairs skip + success with `closeFullTurn`). With **`SRX_LOG_AI_TIMING=1`**, optional **`doorWave`** JSON lines are emitted for each wave.
...
**`SystemSettings`** singleton (`id = "default"`): optional `geminiApiKey`, `geminiModel` (default `gemini-2.5-flash`); **`doorAiDecideBatchSize`**, **`geminiMaxConcurrent`**, **`doorAiMaxConcurrentMcts`**, **`doorAiMoveTimeoutMs`** (defaults **4**, **4**, **1**, **60000**). Postgres connection is **not** stored in the database.
