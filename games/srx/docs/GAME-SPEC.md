...
**SRE-style “door game” play:** a **calendar round** (shown as **day** `dayNumber`) lasts until **every** active empire has used all of its **full turns** for that round (`actionsPerDay`, default **5**), **or** the **round timer** fires first. **Everyone can play at once** — humans are **not** blocked while AIs still owe daily full turns. **`GET /api/game/status`** (and the action route after each human turn) call **`enqueueAiTurnsForSession`** to insert `AiTurnJob` rows when any AI still has daily slots left; the standalone **ai-worker** process claims and executes those jobs so AIs catch up while humans play.
...
- **`POST /api/game/tick`** opens a full turn (`openFullTurn`) when allowed. **`POST /api/game/action`** is used for all actions (including simultaneous); mutating requests take a per-session **MySQL advisory row-level lock** (`INSERT IGNORE SessionLock` + `SELECT … FOR UPDATE NOWAIT`) inside a short interactive transaction; **409** + `galaxyBusy` if the galaxy is busy.
- **`turnsPlayed`:** increments once per economy tick (once per **full turn** opened), not once per mid-turn action. Also increments when turns are **forfeited** by round timeout — forfeited turns count as played so `turnsPlayed + turnsLeft` always equals the original total.
...
- **Round rollover (`tryRollRound`):** after optional timeout forfeit (charging `turnsLeft` and incrementing `turnsPlayed` for skipped slots as above), when every active empire has `fullTurnsUsedThisRound >= actionsPerDay`, resets `fullTurnsUsedThisRound`, `tickProcessed`, `turnOpen` for all empires in the session; increments `dayNumber`; sets **`roundStartedAt`** to now; emits **`GameEvent`** `day_complete`. **After** `day_complete`, when `scheduleAiDrain: true` (default for HTTP routes), **`tryRollRound`** calls `enqueueAiTurnsForSession` (fire-and-forget, outside the `withCommitLock` interactive transaction) to insert `AiTurnJob` rows for every AI that owes daily slots. The external **ai-worker** picks these up and runs them. Simulation paths use `scheduleAiDrain: false` and call `drainDoorGameAiTurns` directly (in-process, synchronous).
...
- **AI (job-queue path):** After any human action or `GET /api/game/status`, `enqueueAiTurnsForSession` inserts one `AiTurnJob` row per AI that still owes daily slots (dedup: skips players with a pending/claimed job). The standalone **ai-worker** process claims jobs via `SELECT … FOR UPDATE SKIP LOCKED`, runs `runOneDoorGameAI` (one AI full turn), marks the job done, then cascades by calling `enqueueAiTurnsForSession` again for any remaining AIs. Multiple worker processes can run concurrently — each claims a different job. Stale jobs (claimed but not completed, e.g. after a crash) are reset to pending every 30 s. **AI (simulation/internal path):** `drainDoorGameAiTurns` (via `runDoorGameAITurns`) is still used by the simulation harness (`scheduleAiDrain: false`) and runs in-process synchronously. **Parallel decide:** up to **`doorAiDecideBatchSize`** (defaults **4**; **`SystemSettings`** via **`/admin`**, else env **`DOOR_AI_DECIDE_BATCH_SIZE`**) eligible AIs have **`getAIMoveDecision`** (Gemini / MCTS / fallback) overlapped with **`Promise.all`** after each has **`openFullTurn`** applied **serially**; **`processAction`** / **`closeFullTurn`** still run **one empire at a time** in that order. Eligible AIs are ordered by **fewest `fullTurnsUsedThisRound` first**, then **`turnOrder` ascending** — fair interleaving. **Concurrency caps (global):** **`geminiMaxConcurrent`** / **`doorAiMaxConcurrentMcts`** in **`SystemSettings`** (admin), else env **`GEMINI_MAX_CONCURRENT`** (default **4**) and **`DOOR_AI_MAX_CONCURRENT_MCTS`** (default **1**). Each AI move is capped by **`doorAiMoveTimeoutMs`** (admin), else env **`DOOR_AI_MOVE_TIMEOUT_MS`** (default **60s**). If `processAiMoveOrSkip` takes the **skip** path (invalid action → `end_turn`), **`closeFullTurn` must still run** — or `turnOpen` stays true, `fullTurnsUsedThisRound` never increments, and the round stalls (`applyDoorGameAIMove` pairs skip + success with `closeFullTurn`). With **`SRX_LOG_AI_TIMING=1`**, optional **`doorWave`** JSON lines are emitted for each wave.
- **Repair:** `scripts/repair-door-game-session.ts` (`npm run repair:door-session`) can detect empires stuck with `turnOpen` + last `TurnLog` `end_turn` + `tickProcessed` false (skip path where `closeFullTurn` never ran). It does **not** match a valid state where `/tick` opened the next full turn (`tickProcessed` true) but the newest log is still the previous `end_turn`.
...
Environment: `ADMIN_USERNAME` / `INITIAL_ADMIN_PASSWORD` (defaults **`admin`** / **`srxpass`**), optional `ADMIN_SESSION_SECRET` for signing the session cookie (defaults align with initial password). **`DATABASE_URL`** is **only** read from the environment (required for Prisma and migrations). **`GEMINI_API_KEY`** / **`GEMINI_MODEL`** can be overridden by **`SystemSettings`** when set. Door-game AI limits (`DOOR_AI_DECIDE_BATCH_SIZE`, `GEMINI_MAX_CONCURRENT`, `DOOR_AI_MAX_CONCURRENT_MCTS`, `DOOR_AI_MOVE_TIMEOUT_MS`) use env when no row exists; **`SystemSettings`** overrides env when a row is stored (same fields as **`/admin`**).

**`SystemSettings`** singleton (`id = "default"`): optional `geminiApiKey`, `geminiModel` (default `gemini-2.5-flash`); **`doorAiDecideBatchSize`**, **`geminiMaxConcurrent`**, **`doorAiMaxConcurrentMcts`**, **`doorAiMoveTimeoutMs`** (defaults **4**, **4**, **1**, **60000**). `DATABASE_URL` is **not** stored in the database — environment only.
